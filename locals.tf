locals {
  ...
  build_command      = local.is_linux ? "cd ${var.code_dir} && GOOS=linux GOARCH=amd64 CGO_ENABLED=0 GOFLAGS=-trimpath go build -mod=readonly -ldflags='-s -w' -o \"${abspath(local.build_output_file)}\" \"${abspath(local.build_input_file)}\"" : "$Env:GOOS=\"linux\"; $Env:GOARCH=\"amd64\"; cd \"${var.code_dir}\"; go build -o \"${abspath(local.build_output_file)}\" \"${abspath(local.build_input_file)}\""
}
